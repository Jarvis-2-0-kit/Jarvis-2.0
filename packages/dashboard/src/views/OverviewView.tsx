  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 12 }}>
        <StatBox label="Agents Online" value={String((detailedHealth?.agents ?? []).filter(a => a.alive).length)} color="var(--green-bright)" />
        <StatBox label="Agents Offline" value={String((detailedHealth?.agents ?? []).filter(a => !a.alive).length)} color="var(--red-bright)" />
