...
const PLATFORMS = [
  ...
  { id: 'reddit', label: 'Reddit', icon: Share2, color: '#FF4500' },
] as const;
...
const PLATFORM_KEY_FIELDS: Record<string, { envKey: string; label: string }[]> = {
  ...
  reddit: [
    { envKey: 'REDDIT_CLIENT_ID', label: 'Client ID (script app)' },
    { envKey: 'REDDIT_CLIENT_SECRET', label: 'Client Secret' },
    { envKey: 'REDDIT_USERNAME', label: 'Reddit Username' },
    { envKey: 'REDDIT_PASSWORD', label: 'Reddit Password' },
  ],
  // ── Media Generation APIs ──
  flux: [{ envKey: 'FLUX_API_KEY', label: 'fal.ai API Key (Flux Pro images)' }],
  kling: [{ envKey: 'KLING_API_KEY', label: 'Kling 3.0 API Key (video generation)' }],
  elevenlabs: [{ envKey: 'ELEVENLABS_API_KEY', label: 'ElevenLabs API Key (voice/TTS)' }],
  heygen: [{ envKey: 'HEYGEN_API_KEY', label: 'HeyGen API Key (avatar videos)' }],
  runway: [{ envKey: 'RUNWAY_API_KEY', label: 'Runway Gen-4 API Key (cinematic video)' }],
  // ── Email & Lead APIs ──
  brevo: [{ envKey: 'BREVO_API_KEY', label: 'Brevo API Key (email automation)' }],
  resend: [{ envKey: 'RESEND_API_KEY', label: 'Resend API Key (email fallback)' }],
  apollo: [{ envKey: 'APOLLO_API_KEY', label: 'Apollo.io API Key (B2B lead enrichment)' }],
};

const MEDIA_APIS = [
  { id: 'flux', label: 'Flux Pro (Images)', icon: Image, color: '#8B5CF6' },
  { id: 'kling', label: 'Kling 3.0 (Video)', icon: Film, color: '#EC4899' },
  { id: 'elevenlabs', label: 'ElevenLabs (Voice)', icon: Zap, color: '#F59E0B' },
  { id: 'heygen', label: 'HeyGen (Avatar Video)', icon: Video, color: '#10B981' },
  { id: 'runway', label: 'Runway Gen-4 (Cinematic)', icon: Film, color: '#6366F1' },
] as const;

const OTHER_APIS = [
  { id: 'brevo', label: 'Brevo (Email)', icon: Send, color: '#0096FF' },
  { id: 'resend', label: 'Resend (Email)', icon: Send, color: '#000000' },
  { id: 'apollo', label: 'Apollo.io (Leads)', icon: Eye, color: '#7C3AED' },
] as const;
      ...
      {tab === 'connect' && (
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          <div style={{
            ...
            color: '#8B5CF6', marginTop: 20, marginBottom: 8,
            borderBottom: '1px solid rgba(139,92,246,0.2)', paddingBottom: 4,
          }}>
            MEDIA GENERATION APIs
          </div>
          {MEDIA_APIS.map((p) => {
            ...
            return (
              <div key={p.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                  <Icon size={16} color={p.color} />
                  <span style={{ fontFamily: 'var(--font-display)', fontSize: 11, letterSpacing: 1.5, color: p.color }}>
                    {p.label.toUpperCase()}
                  </span>
                  {isConnected && (
                    <span style={{
                  ...
                  {fields.map((f) => (
                    <div key={f.envKey}>
                      ...
                      <input
                        type="password"
                        value={keyInputs[p.id]?.[f.envKey] ?? ''}
                        onChange={(e) => setKeyInputs((prev) => ({ ...prev, [p.id]: { ...prev[p.id], [f.envKey]: e.target.value } }))}
                        placeholder={isConnected ? '••••••• (already set)' : 'Enter key...'}
                        style={{
                          width: '100%', fontSize: 11, padding: '5px 10px', marginTop: 3,
                          background: 'var(--bg-primary)', border: '1px solid var(--border-primary)',
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 12, alignItems: 'center' }}>
                  ...
                  {saveMsg?.platform === p.id && (
                    <span style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: saveMsg.ok ? 'var(--green-bright)' : 'var(--red-bright)' }}>
                      {saveMsg.text}
                    </span>
                  )}
                </div>
          ...
          <div style={{
            fontFamily: 'var(--font-display)', fontSize: 10, letterSpacing: 2,
            color: '#0096FF', marginTop: 20, marginBottom: 8,
            borderBottom: '1px solid rgba(0,150,255,0.2)', paddingBottom: 4,
          }}>
            EMAIL & LEAD GENERATION APIs
          </div>
          {OTHER_APIS.map((p) => {
            const Icon = p.icon;
            const fields = PLATFORM_KEY_FIELDS[p.id] ?? [];
