...
export function useGatewayToasts() {
  ...
  useEffect(() => {
    ...
    // --- System alerts (active ping failures, etc.) ---
    unsubs.push(
      gateway.on('system.alert', (payload) => {
        const alert = payload as { severity?: string; title?: string; message?: string };
        addToast({
    ...
    // --- Task dispatch timeout ---
    unsubs.push(
      gateway.on('task.timeout', (payload) => {
        const info = payload as { taskId: string; attempt: number; maxAttempts: number };
        const isFinal = info.attempt >= info.maxAttempts;
        addToast({
          type: isFinal ? 'error' : 'warning',
          title: isFinal ? 'Task Failed' : 'Task Retry',
          message: isFinal
            ? `${info.taskId} failed after ${info.maxAttempts} retries`
            : `${info.taskId} timeout, retry ${info.attempt}/${info.maxAttempts}`,
          duration: isFinal ? 8000 : 5000,
        });
      })
    );
