...
const TASK_DISPATCH_TIMEOUT_MS = 60_000;
...
export class DependencyOrchestrator {
  ...
  private timeoutCallback: ((taskId: string, attempt: number, maxAttempts: number) => void) | null = null;
  ...
  /** Set the callback for dispatch timeout events */
  onTimeout(callback: (taskId: string, attempt: number, maxAttempts: number) => void): void {
  ...
  /** Check for assigned tasks that timed out waiting for agent to start */
  private checkDispatchTimeouts(): void {
    const now = Date.now();
    for (const node of this.graph.nodes.values()) {
      if (node.status !== 'assigned' || !node.dispatchedAt) continue;
      if (now - node.dispatchedAt < TASK_DISPATCH_TIMEOUT_MS) continue;

      node.retryCount++;
      this.timeoutCallback?.(node.taskId, node.retryCount, MAX_DISPATCH_RETRIES);

      if (node.retryCount < MAX_DISPATCH_RETRIES) {
        log.warn(`Task ${node.taskId} dispatch timeout (attempt ${node.retryCount}/${MAX_DISPATCH_RETRIES}), re-queuing`);
        node.status = 'ready';
        node.assignedAgent = undefined;
        node.dispatchedAt = undefined;
      } else {
        log.error(`Task ${node.taskId} failed after ${MAX_DISPATCH_RETRIES} dispatch retries`);
        this.failTask(node.taskId, `Dispatch timeout after ${MAX_DISPATCH_RETRIES} retries`);
      }
    }
  }
