...
# Copy node's dynamic libraries (node@22 from Homebrew is dynamically linked)
NODE_LIB_DIR="$(dirname "$(dirname "$(realpath "$NODE_BIN")")")/lib"
if [ -f "$NODE_LIB_DIR/libnode.127.dylib" ]; then
    cp -L "$NODE_LIB_DIR/libnode.127.dylib" "$APP_BUNDLE/Contents/MacOS/"
    echo "  Copied libnode.127.dylib"
fi
...
DMG_PATH="$SCRIPT_DIR/Jarvis-2.0.dmg"
rm -f "$DMG_PATH"
hdiutil create -volname "Jarvis 2.0" -srcfolder "$APP_BUNDLE" -ov -format UDZO "$DMG_PATH" 2>&1 | tail -1
