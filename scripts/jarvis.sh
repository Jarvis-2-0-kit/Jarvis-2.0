...
ALPHA_PASS_CFG="${SMITH_PASS:-${VNC_ALPHA_PASSWORD:-}}"
BETA_PASS_CFG="${JOHNY_PASS:-${VNC_BETA_PASSWORD:-}}"
_agent_host()    { eval echo "\${${1}_HOST_CFG}"; }
_agent_user()    { eval echo "\${${1}_USER_CFG}"; }
_agent_id()      { eval echo "\${${1}_AGENT_ID}"; }
_agent_pass()    { eval echo "\${${1}_PASS_CFG}"; }

expect_ssh() {
  local user="$1" host="$2" pass="$3" cmd="$4"
  /usr/bin/expect <<EXPECT_SCRIPT
set timeout 30
spawn ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 ${user}@${host} "${cmd}"
expect {
...
ssh_to_agent() {
  ...
  local pass; pass=$(_agent_pass "$upper")
  [[ -z "$host" ]] && { echo -e "  ${RED}✗${RESET} ${name}: no host configured"; return 1; }
  expect_ssh "$user" "$host" "$pass" "$cmd"
}
...
agents_start() {
  ...
  for name in alpha beta; do
    ...
    # 1. Kill stale remote processes (fire-and-forget)
    expect_ssh "$user" "$host" "$pass" \
      "pkill -f 'tsx.*agent-runtime/src/cli.ts' 2>/dev/null; pkill -f 'node.*agent-runtime/src/cli.ts' 2>/dev/null; pkill -f vnc-proxy 2>/dev/null; true" \
      </dev/null 2>/dev/null || true

    # 2. Kill old persistent SSH PIDs (both -agent and -vnc)
    for suffix in agent vnc; do
      local pid_file="/tmp/jarvis-ssh-${agent_id}-${suffix}.pid"
      if [[ -f "$pid_file" ]]; then
    ...
    # 3. Start VNC proxy via persistent SSH + expect (background)
    local vnc_log="/tmp/jarvis-ssh-${agent_id}-vnc.log"
    local vnc_pid_file="/tmp/jarvis-ssh-${agent_id}-vnc.pid"
    nohup /usr/bin/expect <<VNC_EXPECT > "$vnc_log" 2>&1 &
set timeout -1
spawn ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 -o ServerAliveInterval=10 -o ServerAliveCountMax=30 ${user}@${host} "export PATH=\\\$HOME/.local/bin:/opt/homebrew/bin:/usr/local/bin:\\\$PATH && cd ~/Documents/Jarvis-2.0/jarvis && set -a && source .env 2>/dev/null && set +a && export VNC_USERNAME=${user} && export VNC_PASSWORD='${pass}' && exec ./node_modules/.bin/tsx scripts/vnc-proxy.ts"
expect {
    "*assword*" { send "${pass}\r"; exp_continue }
    eof {}
}
VNC_EXPECT
    ...
    # 4. Start agent-runtime via persistent SSH + expect (background)
    # macOS revokes network for orphaned processes — keeping SSH alive prevents this
    local agent_log="/tmp/jarvis-ssh-${agent_id}-agent.log"
    local agent_pid_file="/tmp/jarvis-ssh-${agent_id}-agent.pid"
    nohup /usr/bin/expect <<AGENT_EXPECT > "$agent_log" 2>&1 &
set timeout -1
spawn ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 -o ServerAliveInterval=10 -o ServerAliveCountMax=30 ${user}@${host} "export PATH=\\\$HOME/.local/bin:/opt/homebrew/bin:/usr/local/bin:\\\$PATH && source ~/.nvm/nvm.sh 2>/dev/null && nvm use 22 2>/dev/null; cd ~/Documents/Jarvis-2.0/jarvis && set -a && source .env && set +a && export ANTHROPIC_AUTH_MODE=\\\${ANTHROPIC_AUTH_MODE:-claude-cli} && security unlock-keychain -p \\\${KEYCHAIN_PASSWORD:-137009} ~/Library/Keychains/login.keychain-db 2>/dev/null; exec node_modules/.bin/tsx packages/agent-runtime/src/cli.ts"
expect {
    "*assword*" { send "${pass}\r"; exp_continue }
    eof {}
}
AGENT_EXPECT
    local agent_ssh_pid=$!
    echo "$agent_ssh_pid" > "$agent_pid_file"

    # 5. Verify after 3s
    sleep 3
    local running=0
    if kill -0 "$agent_ssh_pid" 2>/dev/null; then
      running=1
    fi
    if [[ $running -eq 1 ]]; then
      ok "Agent running via persistent SSH (PID: ${agent_ssh_pid})"
    else
      fail "SSH session died — check: ${agent_log}"
    fi
    if [[ -f "$vnc_pid_file" ]] && kill -0 "$(cat "$vnc_pid_file")" 2>/dev/null; then
      ok "VNC proxy running via persistent SSH"
    else
      warn "VNC proxy may not have started — check: ${vnc_log}"
    fi
  done
...
agents_stop() {
  ...
  for name in alpha beta; do
    ...
    # Kill remote processes via expect
    ssh_to_agent "$name" "pkill -f agent-runtime; pkill -f vnc-proxy; true" 2>/dev/null \
      && ok "Remote processes killed on ${name}" \
      || warn "Could not reach ${name}"

    # Kill persistent SSH PIDs (both -agent and -vnc suffixes)
    for suffix in agent vnc; do
      local pid_file="/tmp/jarvis-ssh-${agent_id}-${suffix}.pid"
      if [[ -f "$pid_file" ]]; then
        kill "$(cat "$pid_file")" 2>/dev/null || true
        rm -f "$pid_file"
        ok "Killed local SSH-${suffix} session"
      fi
    done
  done
